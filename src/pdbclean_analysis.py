...
def initialize(projdir='./', source=None, target=None):
    """
    initialize
    """
    ciflist = glob.glob(projdir+'/'+source+'/*.cif')
    pdblist = []
    for ciffile in ciflist:
        pdbfile=projdir+'/'+target+'/'+ciffile.split('/')[-1].split('.cif')[0]+'.pdb'
        if not os.path.exists(pdbfile):
            cif.cif2pdb(ciffile, pdbfile)
        pdblist.append(pdbfile)
    t = mdtraj.load(pdblist)
    traj = t.superpose(t, 0)
    return traj
...
def cluster(traj,pc_thresh=0.75,fun='logcosh',algo='parallel',analysis_type='pca',do_plot=True,title=''):
    ...
    """
    distance, x, weight = get_distance(traj,analysis_type=analysis_type,pc_thresh=pc_thresh,fun=fun,algo=algo) 
    clusters = scipy.cluster.hierarchy.linkage(distance, method='ward')
    if(do_plot):
        plot_cluster(clusters,x,figname=title)
    return clusters

def get_distance(traj,analysis_type='ica',pc_thresh=0.75,fun='logcosh',algo='parallel'):
    ...
    if(analysis_type=='ica'):
        atype='ica'
    v,m,x = analyses(traj,pc_thresh=pc_thresh,fun=fun,algo=algo,analysis_type=atype,do_plot=False)
...
def analyses(traj,pc_thresh=0.75,fun='logcosh',algo='parallel',analysis_type='ica',do_plot=True,c=None,title=''):
    ...
    """
    v_pca, l_pca, x_pca = traj2pc(traj, n_components=traj.xyz.shape[0],var_trunc=pc_thresh)
    x = x_pca
    if(do_plot):
        print('Principal Component Analysis (step 0)')
        score = score_mode(traj,v=None,pc_thresh=pc_thresh,analysis_type='pca')
    ...
    if(analysis_type == 'ica'):
        ...
        if(len(l_pca)>1 and do_plot):
            ...
            score = score_mode(traj,v=v_ica,pc_thresh=pc_thresh)
...
def save_mode(traj,prj,n=np.arange(1),v=None,pc_thresh=0.75,keyword='mode',movie='oscillatory',nframe=20,verbose='minimal',analysis_type='ica'):
    ...
    xyz_mode = get_mode(traj, v=v, pc_thresh=pc_thresh,analysis_type=analysis_type)
    for ic in n:
        index = get_sorted_index(prj,iIC=ic,nICs=n)
        traj_IC = traj_slice(traj,index=index)
...
def score_mode(traj,v=None,pc_thresh=0.75,score_type='elasticity',analysis_type='ica'):
    """ score_mode
    """
    xyz_mean, b_factors = get_xyz_mean(traj)
    xyz_mode = get_mode(traj, v=v, pc_thresh=pc_thresh,analysis_type=analysis_type)
...
def save_traj(traj,keyword='traj',save_mean=False,verbose='minimal'):
...
def cluster_split(traj,l,n_clusters,title='',save_mean=True):
    ...
    for i_cluster in np.arange(1,n_clusters+1):
        ...
        save_traj(traj.slice(idx_in),keyword=keyword,save_mean=save_mean,verbose='full')
...
def traj_slice(traj,index=[]):
    if(len(index) == 0):
        index = np.arange(traj.xyz.shape[0])
    return traj.slice(index)
